...
def plot(variab, func, restr, dots):
    ...
    if restr[x] == [-np.inf, np.inf]:  # определяем ограничения по осям
        ...
        x_min = x_min - max(0.1 * (x.max - x.min), 1)  # во избежание отрисовки точек на границах графика
        x_max = x_max + max(0.1 * (x.max - x.min), 1)
    else:
        x_min = restr[x][0]
        x_max = restr[x][1]
    ...
    if restr[y] == [-np.inf, np.inf]:
        ...
        y_min = y_min - max(0.1 * (y.max - y.min), 1)
        y_max = y_max + max(0.1 * (y.max - y.min), 1)
    else:
        y_min = restr[y][0]
        y_max = restr[y][1]
